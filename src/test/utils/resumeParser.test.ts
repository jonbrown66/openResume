...
`.trim();
  ...
  it('preserves markdown tables during auto-formatting without stripping pipe chars or adding bullets', () => {
    const rawMarkdown = `---
name: JANE DOE
title: Developer
---
...
｜ :--- ｜ :--- ｜ :--- ｜
｜ Open Source ｜ 2021 - Present ｜ Contributor ｜
｜ Mentoring ｜ 1 Year ｜ Advisor ｜
`.trim();

    const formatted = autoFormatResume(rawMarkdown, 'en');

    // Test in skills-like section
    expect(formatted).toContain('| Tool | Proficiency |');
    expect(formatted).toContain('| :--- | :--- |');
    expect(formatted).toContain('| **React** | Expert |');
    expect(formatted).not.toContain('- | **React** | Expert |');

    // Test inside nested block experience entry
    expect(formatted).toContain('| Metric | Value |');
    expect(formatted).toContain('| Success Rate | 98% |');
    expect(formatted).not.toContain('- | Success Rate | 98% |');

    // Test table starting directly on first line under a non-skills section, and with full-width pipes
    expect(formatted).toContain('｜ Activity ｜ Duration ｜ Role ｜');
    expect(formatted).toContain('｜ :--- ｜ :--- ｜ :--- ｜');
    expect(formatted).toContain('｜ Open Source ｜ 2021 - Present ｜ Contributor ｜');
    expect(formatted).not.toContain('- ｜ Open Source ｜');
  });
});
